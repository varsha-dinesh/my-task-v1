...
function AddTask() {
  ...
  const handleAddComponent = () => {
    setComponents((prevComponents) => [...prevComponents, { id: prevComponents.length }]);
  };
  ...
  return (
    <>
      <Input onClick={() => setOpenTask(true)} task={tasks} />
      {openTask && (
          ...
          <form onSubmit={handleFormSubmit}>
            ...
            {isOpen && (
              <div ref={calendarRef} className="relative w-[250px] z-[9999]">
                <div className="absolute z-[99999] h-3 w-3 rotate-45 mt-[0px] ml-[4.2rem] border-l border-t border-[#e3e8ee] bg-white"></div>
                <Calendar
                  onChange={handleDateChange}
                  value={selectedDate}
